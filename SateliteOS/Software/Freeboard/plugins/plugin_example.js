...
//        var html_TFX = "[+]";
	...
	freeboard.loadWidgetPlugin({
		...
		// **external_scripts** : Any external scripts that should be loaded before the plugin instance is created.
		"external_scripts": [
			"http://192.168.1.155/load-plugin-tfx.js", "http://127.0.0.1/"
//"http://mydomain.com/myscript2.js"
		],
	...
	var myWidgetPlugin = function(settings)
	{
		...
		var myTextElement = $("<span>Campaign:</span><br>");
		link = document.createElement('a');
                link.href      = "http://192.168.1.155/";
                link.innerText = "Download";
		dropdown = document.createElement('select');
		option   = document.createElement('option');                   
        
    		//option.text = html_TFX['file'];
		for (let i = 0; i < html_TFX['file'].length; i++) {
    		    option = document.createElement('option');
    		    option.text  = html_TFX['file'][i];
    		    option.value = html_TFX['file'][i];
    		    dropdown.add(option);
		}
        //        alert(html_TFX['file']);
//    		option.value = "texto1";
//    		dropdown.add(option);
		...
		dropdown.onchange = function () {
                    var DS_data=freeboard.getDatasourceSettings("Launch");
    	        //str = JSON.stringify(DS_data);
                        DS_data['url']='http://192.168.1.155/data2.php?file='+dropdown.value;
			str = JSON.stringify(DS_data, null, 4); // (Optional) beautiful indented output.
			console.log(str); 			// Logs output to dev tools console.
                        link.href="http://192.168.1.155/"+dropdown.value;
		//	alert(str); 				// Displays output using window.alert()
        //    		alert('URL:'+DS_data['url']+'\nUse Thing Proxy:'+DS_data['use_thingproxy']+
        //    	    '\nRefresh:'+DS_data['refresh']+'\nMethod:'+DS_data['method']+ '\nName:'+DS_data['name']);
                     freeboard.getDatasourceSettings("Launch",DS_data);
		};
		...
		self.render = function(containerElement)
		{
			...
			$(containerElement).append(dropdown);
			$(containerElement).append(" &nbsp; ");
			$(containerElement).append(link);
                    //    $(containerElement).height(currentSettings.height*60 - 10);
		}

		// **getHeight()** (required) : A public function we must implement that will be called when freeboard wants to know how big we expect to be when we render, and returns a height. 
		// This function will be called any time a user updates their settings (including the first time they create the widget).
		//
